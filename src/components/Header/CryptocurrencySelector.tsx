...
import { addSelectedCrypto, removeSelectedCrypto } from '../../store/slices/cryptoSlice';

export default function CryptocurrencySelector() {
  const [isOpen, setIsOpen] = useState(false);
  const dispatch = useDispatch();
  
  const { cryptocurrencies, selectedCrypto } = useSelector((state: RootState) => state.crypto);
  ...
  const getSelectedCryptoNames = () => {
    const selected = cryptocurrencies.filter(crypto => selectedCrypto.includes(crypto.id));
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center space-x-2 px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors min-w-[160px]"
      >
        <span className="text-sm font-medium text-gray-700 truncate">
          {getSelectedCryptoNames()}
        </span>
        <ChevronDown className={`w-4 h-4 text-gray-500 transform transition-transform flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-1 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
          <div className="p-3 border-b border-gray-100">
            <h3 className="text-sm font-medium text-gray-900">Select Cryptocurrencies</h3>
            <p className="text-xs text-gray-500 mt-1">Choose currencies to display on chart</p>
          </div>
